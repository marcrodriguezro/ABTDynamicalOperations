...
public:
 ...
 // void Dynamic_decoder(int height, 
 //     std::vector<int> &str, std::vector<std::pair<int, int>> *edgesReconstructed);
 
  void Partial_decompression(std::string result, int u, int v, int n);

 // void DecodeBit(std::string result, std::vector<int> &str);
...
private:
  ...
  inline bool Exist(const std::vector<std::vector<int>> &adj,
                    int from, int to);
  inline bool Contains(const std::vector<std::vector<int>>& adj, int from, int to);
...
inline bool AbtCompression ::Exist(const std::vector<std::vector<int>> &adj, int from, int to) // se ha corregido el &adj para que permita acceder al valor en vez de a la posicion de memoria
{
  return std::binary_search(adj[from].begin(), adj[from].end(), to);

}
inline bool AbtCompression::Contains(const std::vector<std::vector<int>>& adj, int from, int to) { //MINE
    if (adj[from][to] == 1) {
        return true;
    }
    else {
